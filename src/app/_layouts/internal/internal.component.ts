...
export class InternalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.util.observTheme().subscribe((_res) => {
      ...
      this.iframURl = this.sanitize.bypassSecurityTrustResourceUrl(
        'https://timesprayer.com/widgets.php?frame=2&lang=en&name=sharijah&avachang=true&time=0&fcolor=' +
          this.prayerTheme.secondary +
          '&tcolor=' +
          this.prayerTheme.menuText +
          '&frcolor=' +
          this.prayerTheme.secondary
      );
    });
  ...
  getLangs() {
    this.service.get(apis.languages, '').subscribe((_res) => {
      ...
      this.languages.forEach((_lang: { name: any; code: any }) => {
        labels.push({
          label: _lang.name,
          command: (Event: Event) => this.setLang(_lang.code),
        });
      });
